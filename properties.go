...
type PropertiesConnect struct {
	...
	RequestProblemInfo byte `json:"request_problem_info"`
}
...
func (p *PropertiesConnect) size() int {
	...
	size += sizePropSessionExpiryInterval(p.Flags, p.SessionExpiryInterval)
	size += sizePropReceiveMaximum(p.Flags, p.ReceiveMaximum)
	size += sizePropMaxPacketSize(p.Flags, p.MaximumPacketSize)
	size += sizePropTopicAliasMaximum(p.Flags, p.TopicAliasMaximum)
	size += sizePropRequestProblemInfo(p.Flags, p.RequestProblemInfo)
	size += sizePropRequestResponseInfo(p.Flags, p.RequestResponseInfo)
...
func (p *PropertiesWill) size() int {
	...
	size += sizePropWillDelayInterval(p.Flags, p.WillDelayInterval)
	size += sizePropPayloadFormatIndicator(p.Flags, p.PayloadFormatIndicator)
	size += sizePropMessageExpiryInterval(p.Flags, p.MessageExpiryInterval)
...
func sizePropSessionExpiryInterval(flags propertyFlags, interval uint32) int {
	if flags.has(PropertySessionExpiryInterval) {
		// Size of the field + 1 byte for the property identifier
		return sizeUint(interval) + 1
	}
	return 0
}
...
func sizePropReceiveMaximum(flags propertyFlags, val uint16) int {
	if flags.has(PropertyReceiveMaximum) {
		// Size of the field + 1 byte for the property identifier
		return sizeUint(val) + 1
	}
	return 0
}
...
func sizePropMaxPacketSize(flags propertyFlags, val uint32) int {
...
func sizePropTopicAliasMaximum(flags propertyFlags, val uint16) int {
...
func sizePropRequestResponseInfo(flags propertyFlags, val byte) int {
...
func decodePropRequestResponseInfo(buf []byte, p properties) (v byte, n int, err error) {
	if p.Has(PropertyRequestResponseInfo) {
		return v, n, ErrMalformedPropertyRequestResponseInfo
	}

	n, err = decodePropUint[uint8](buf, &v, func(val byte) bool { return val == 0 || val == 1 })
	if err != nil {
		return v, n, ErrMalformedPropertyRequestResponseInfo
	}
...
func sizePropRequestProblemInfo(flags propertyFlags, val byte) int {
...
func decodePropRequestProblemInfo(buf []byte, p properties) (v byte, n int, err error) {
...
func sizePropWillDelayInterval(flags propertyFlags, val uint32) int {
...
func sizePropPayloadFormatIndicator(flags propertyFlags, val byte) int {
...
func decodePropPayloadFormatIndicator(buf []byte, p properties) (v byte, n int, err error) {
	if p.Has(PropertyPayloadFormatIndicator) {
		return v, n, ErrMalformedPropertyPayloadFormatIndicator
	}

	n, err = decodePropUint[byte](buf, &v, func(val byte) bool { return val == 0 || val == 1 })
	if err != nil {
		return v, n, ErrMalformedPropertyPayloadFormatIndicator
	}
...
func sizePropMessageExpiryInterval(flags propertyFlags, val uint32) int {
